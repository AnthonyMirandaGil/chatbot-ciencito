...
#from service.qa import get_answer
...
class QuestionAnsweringAction(Action):
    def name(self) -> Text:
        return "action_question_answering_system"
    
    def run(self, dispatch, tracker, domain):
        question = tracker.latest_message['entities']
        #answer = get_answer(question)
        ## llamar a api question answering
        answer = 'Aqui esta la **respuesta**'
        #answer = "Aqui esta la <span style="background-color: #FFFF00">respuesta</span>"
        dispatch.utter_message(answer)
        return []
